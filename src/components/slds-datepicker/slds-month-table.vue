	...
	<table aria-labelledby="month" aria-multiselectable="true" class="slds-datepicker__month" role="grid">
		...
		<tbody>
			<tr v-for="(week,index) in dateMatrix" :key="index">
				<td v-for="day in week" :key="day.getTime()" role="gridcell" :class="{'slds-disabled-text': !isDayOfThisMonth(day), 'slds-is-selected': isSelected(day)}" @click="isDayOfThisMonth(day)? selectDate(day) : undefined">
...
export default {
    ...
    methods: {
        ...
        isSelected: function(date) {
            ...
            if (
                this.value.getUTCDate() === date.getUTCDate() &&
