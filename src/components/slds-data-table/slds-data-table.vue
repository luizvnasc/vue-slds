	...
	<div>
		<table 
			...
			<thead>
				<tr>
					<th 
						...
						<a 
							class="slds-th__action slds-text-link_reset slds-align_absolute-center" 
							role="button" 
							tabindex="-1" 
							@click="!!column.sortable ? sortColumn(column): undefined ">
							...
							<div 
								...
								<arrow-down-icon 
									...
									class="slds-icon slds-icon_x-small slds-icon-text-default slds-is-sortable__icon" />
								<arrow-up-icon 
									v-if="column.sortable === 'asc'" 
									class="slds-icon slds-icon_x-small slds-icon-text-default slds-is-sortable__icon" />
							</div>
			...
			<tbody>
				...
				<slot 
					v-for="row in rowRange" 
					v-if="hasCustomRow && dataset.length > 0" 
					:row="dataset[row]"/>
		...
		<div 
			v-if="options.pagination === true" 
			class="slds-border_top slds-border_bottom slds-p-around_x-small slds-grid">
				...
				<ul class="slds-button-group-list slds-align_absolute-center">
					<li>
						<button 
							...
							@click="previousPages()">&lt;&lt;</button>
					</li>
					<li>
						<button 
							:disabled="!hasPreviousPage" 
							class="slds-button slds-button_neutral" 
							@click="pageIndex > 0 ? pageIndex--: 0 ">&lt;</button>
					</li>
					<li 
						v-for="i in pageRange" 
						:key="i">
						<button 
							:class="{'slds-button_neutral':i !== pageIndex, 'slds-button_brand':i === pageIndex}" 
							class="slds-button " 
							type="buttom" 
							@click="pageIndex = i">{{ i+1 }}</button>
					</li>
					<li>
						<button 
							:disabled="!hasNextPage" 
							class="slds-button slds-button_neutral" 
							@click="pageIndex < numberOfPages ? pageIndex++: numberOfPages ">&gt;</button>
					</li>
					<li>
						<button 
			...
			<div class="slds-col slds-size_3-of-12">
				<div class="slds-form-element slds-align_absolute-center">
					<label 
						...
						for="text-input-id-1">Row per page</label>
					<div class="slds-form-element__control">
						<select 
							v-model="rowsPerPage" 
							class="slds-input">
							<option 
								v-for="rows in listRowsPerPage" 
								:key="rows" 
