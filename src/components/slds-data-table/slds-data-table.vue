	...
	<div>
		<table :class="{
					...
					<th v-for="column in columns" :key="column.fieldName" :colspan="column.colspan || 0" :class="{'slds-is-sortable': column.sortable}">
						<a class="slds-th__action slds-text-link_reset slds-align_absolute-center" role="button" tabindex="-1" @click="!!column.sortable ? sortColumn(column): undefined ">
							...
							<div v-if="!!column.sortable" class="slds-icon_container">
								<arrow-down-icon v-if="column.sortable !== 'asc'" class="slds-icon slds-icon_x-small slds-icon-text-default slds-is-sortable__icon" />
								<arrow-up-icon v-if="column.sortable === 'asc'" class="slds-icon slds-icon_x-small slds-icon-text-default slds-is-sortable__icon" />
							</div>
						</a>
					</th>
				</tr>
			...
			<tbody>
				...
				<slot v-for="row in rowRange" v-if="hasCustomRow && dataset.length > 0" :row="dataset[row]" />
				<!--default rows if slot is empty -->
				<tr v-for="row in rowRange" v-if="!hasCustomRow && dataset.length > 0" :key="row">
			...
			<div class="slds-col slds-size_6-of-12">
				<ul class="slds-button-group-list slds-align_absolute-center">
					...
					<li v-for="i in pageRange" :key="i">
						<button :class="{'slds-button_neutral':i !== pageIndex, 'slds-button_brand':i === pageIndex}" class="slds-button " type="buttom" @click="pageIndex = i">{{ i+1 }}</button>
					</li>
					<li>
			...
			<div class="slds-col slds-size_3-of-12">
				<div class="slds-form-element slds-align_absolute-center">
					...
					<div class="slds-form-element__control">
						<select v-model="rowsPerPage" class="slds-input">
  ...
  computed: {
    ...
    colspan: function() {
      ...
      for (let i = 0; i < this.columns.length; i++) {
        cols +=
          this.columns[i].colspan !== undefined ? this.columns[i].colspan : 0;
      }
      return cols;
    },
    ...
    pageRange() {
      return this.lastPageIndex == 0
        ? 0
        : _.range(this.firstPageIndex, this.lastPageIndex);
    },
    rowRange() {
      ...
      return _.range(
        firstrow,
        firstrow + this.rowsPerPage >= this.dataset.length
          ? this.dataset.length
          : firstrow + this.rowsPerPage
      );
    }
  },
  watch: {
